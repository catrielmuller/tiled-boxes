...
module.exports = class TiledBoxes {
  ...
  async parseInput(deviceId, elem) {
    if (this.devices[deviceId].currentAppId === 'index') {
      if (elem.element === 'matrix') {
        if (this.devices[deviceId].apps[elem.position.x] && this.devices[deviceId].apps[elem.position.x][elem.position.y] !== null) {
          const enter = await this.devices[deviceId].apps[elem.position.x][elem.position.y].launch();
          if (enter) {
            this.devices[deviceId].currentApp = this.devices[deviceId].apps[elem.position.x][elem.position.y];
  ...
  async showLogo() {
    ...
    const logoRedImg = await images.load(`${this.config.paths.assets.icons.low}red.png`);
    for (const id in this.devices) {
      this.devices[id].emit('image', logoImg);
    }
    await tools.sleep(500);
    for (const id in this.devices) {
      this.devices[id].emit('image', logoRedImg);
    }
    return true;
  }
