...
module.exports = class TiledBoxes {
  constructor() {
    ...
    this.parseOutputWS = this.parseOutputWS.bind(this);
  }
  ...
  async initWebServer() {
    ...
    io.on('connection', (socket) => {
      socket.on('input-ws', (msg) => {
        for (const id in this.devices) {
          for (let x = 0; x < this.devices[id].apps.length; x += 1) {
            for (let y = 0; y < this.devices[id].apps[x].length; y += 1) {
              if (this.devices[id].apps[x][y] !== null) {
                this.devices[id].apps[x][y].emit('input-ws', msg);
              }
            }
          }
        }
  ...
  async parseOutputWS(msg) {
    io.emit('output-ws', msg);
  }
  ...
  async initApps() {
    ...
    for (const id in this.devices) {
      ...
      for (let x = 0; x < this.devices[id].apps.length; x += 1) {
        for (let y = 0; y < this.devices[id].apps[x].length; y += 1) {
          if (this.devices[id].apps[x][y] !== null) {
            this.devices[id].apps[x][y].on('output-ws', this.parseOutputWS);
          }
        }
      }
